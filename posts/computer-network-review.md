...
>
> 系统分层实现的原则：层数应适中，每层功能应明确，层与层应相互独立，层间单向引用
...
>
> - 应用层（application layer）：支持网络应用程序（FTP，SMTP，HTTP）
> - 传输层（transport layer）：实现进程之间的数据传输（TCP，UDP）
> - 网络层（network layer）：将数据报从源端传输到目的端（IP，routing protocols）
> - 链路层（link layer）：将数据帧从一个节点传输到相邻节点（Ethernet, WiFi）
> - 物理层（physical layer）：bits on the wire

<font color="red">OSI 体系结构模型</font>

> ![](/assets/net-fig42.png)
> 
...
>
> OSI 参考模型优点
> - OSI 结构清晰，概念明确，适合作教学内容
> - 产生在协议发明之前，非常通用
>
> OSI 参考模型缺点
> - 设计者没有实际经验，层次功能划分不是十分合理
> - 协议实现复杂，运行效率低
> - 标准制定周期长，符合该标准的设备无法及时进入市场
>
> TCP/IP 参考模型优点
> - 一开始就考虑到异构网互联问题
> - 一开始就对面向连接的服务和无连接的服务并重
> - 有很好的网管功能
>
> TCP/IP 参考模型缺点
> - 对服务、协议等概念并没有很清楚地区分开
