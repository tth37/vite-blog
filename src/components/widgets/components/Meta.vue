...
<template>
  <div class="flex flex-row flex-wrap mt-5 justify-start">
    <router-link
      v-if="props.meta.category"
      :to="`/category/${props.meta.category}`"
    ...
    <router-link
      v-if="props.meta.tags"
      v-for="tag in props.meta.tags"
      :key="tag"
      :to="`/tag/${tag}`"
    ...
    <span
      class="bg-gray-100 text-gray-800 text-xs font-medium inline-flex items-center px-2 py-0.5 rounded mr-2 dark:bg-gray-700 dark:text-gray-400 border border-gray-500 whitespace-nowrap"
    >
      <font-awesome-icon icon="fa-solid fa-clock" class="mr-1" />
      {{ renderTimeAgo(props.meta.date) }}
    </span>
  </div>
</template>
